...
func TestGeminiContent_PlainMarkdown(t *testing.T) {
	body := string(GeminiContent())
	if strings.HasPrefix(body, "---\n") {
		t.Errorf("GEMINI.md should NOT have YAML frontmatter; got:\n%s", body[:80])
	}
	if !strings.HasPrefix(body, "# ") {
		t.Errorf("GEMINI.md should start with a top-level heading; got:\n%s", body[:80])
	}
}

func TestGeminiContent_MentionsBothTools(t *testing.T) {
	body := string(GeminiContent())
...
func TestGeminiContent_DocumentsRejectReasons(t *testing.T) {
	body := string(GeminiContent())
	for _, code := range []string{"secret_detected", "provenance_violation", "target_drift"} {
		if !strings.Contains(body, code) {
			t.Errorf("GEMINI.md missing reject reason %q", code)
		}
	}
}
...
func TestInstall_ProjectLocal(t *testing.T) {
	...
	if !strings.Contains(string(body), "agent-memory") {
		t.Error("installed GEMINI.md doesn't match embedded asset")
	}
}
...
func TestInstall_ForceOverwrites(t *testing.T) {
	root := t.TempDir()
	target := filepath.Join(root, "GEMINI.md")
