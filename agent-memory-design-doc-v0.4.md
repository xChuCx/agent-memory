# Agent Memory Protocol — Design Doc v0.4
...
5. **Concurrency model is explicit.**
   A single-writer advisory lock on `.agent-memory/meta/lock.json` protects all mutating operations. Readers use the on-disk state directly without locks. Stale locks (past TTL) can be broken with a logged warning. The full model is specified in §11.
...
1. Open `lock.json` with `O_CREAT | O_EXCL`.
2. If exists: read it.
   - If `acquired_at + ttl_seconds < now`: stale. Log warning, take over (atomic rename of stale to `lock.json.expired-<timestamp>`, retry from step 1).
   - Otherwise: wait with exponential backoff up to `wait_timeout_seconds` (default 10s). Then return `LockHeld` error to caller.
3. Write owner metadata.
4. Perform work.
5. Release: delete `lock.json` atomically.

### 11.4 Crash Recovery

- Lock with expired TTL is reclaimable by any process.
- On startup, every command runs a `meta/lock.json` sanity check; expired locks are removed.
- `agent-memory doctor` reports lock health.
...
{
  "files": {
    ".agent-memory/modules/auth.md": "sha256:7d3f...",
    ".agent-memory/index.md": "sha256:a2b1..."
  },
...
- On `apply`, server re-checks `target-checksums.json` against current files.
  - On match: proceed.
  - On mismatch (drift): fail with `target_drift`, suggesting `agent-memory rebase <staging_id>` (which re-resolves section IDs against current file state and rewrites the staged content; fails if section was removed).
...
```bash
agent-memory review                       # list staged updates
agent-memory review <staging_id>          # detail one
agent-memory apply <staging_id>           # apply (with drift check)
agent-memory apply <staging_id> --commit  # apply and git commit durable changes
agent-memory reject <staging_id>          # delete staging dir
agent-memory rebase <staging_id>          # re-resolve against current file state
...
agent-memory status
agent-memory review [<staging_id>] [--since 24h]
agent-memory apply <staging_id> [--commit]
agent-memory reject <staging_id>
agent-memory rebase <staging_id>
...
| Concurrent write corruption | High | Single-writer lock (§11) |
